...
# CHANGE BY JYOTI - 11-Sep-2026: Email OTP verification imports
import secrets
import hashlib
import hmac
import time
...
# CHANGE BY JYOTI - 11-Sep-2026: START - Email OTP backend for Telecom Solutions & Services website
# ============================================================
# CONTACT EMAIL OTP
# ============================================================
...
def _send_contact_otp_email(
    ...
    subject = (
        "Telecom Solutions & Services Email Verification Code"
    )
...
# CHANGE BY JYOTI - 11-Sep-2026: END - Email OTP backend
...
def request_demo_view(request):
    ...
    # Detect AJAX/fetch requests used by the Submit Enquiry modal.
    wants_json = (
    ...
    # CHANGE BY JYOTI - 11-Sep-2026:
    # Normalise email and read the OTP verification token.
    email = _normalise_email(
    ...
    subject = "New Telecom Solutions & Services Enquiry"

    text_body = (
        "A new Telecom Solutions & Services enquiry request "
        "was submitted.\n\n"
        f"Submitted: {ts}\n"
    ...
    html_body = f"""
        <h2 style="margin:0 0 8px">
            New Telecom Solutions & Services Enquiry Request
        </h2>
    ...
    # CHANGE BY JYOTI - 11-Sep-2026:
    # Verification is one-use after successful submission.
...
def contact_section(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        # Email body
        subject = "New website contact submission for Telecom Solutions & Services"
        text_body = "\n".join(
            [
                "New contact submission for Telecom Solutions & Services:",
...
def contact_block_submit(request):
    """
    Handles the Telecom Solutions & Services consulting/contact form.
    """
    ...
    # CHANGE BY JYOTI - 11-Sep-2026:
    # Normalise email and read OTP verification token.
    email = _normalise_email(
    ...
    # CHANGE BY JYOTI - 11-Sep-2026:
    # Telecom Solutions & Services email notification.
    subject = (
        f"[Telecom Solutions & Services Website] "
        f"Consulting request: {name}"
    )

    text_body = "\n".join(
        [
            (
                "A new consulting request was "
                "submitted for Telecom Solutions & Services:"
            ),
            f"Name: {name}",
            f"Email: {email}",
            (
                "Phone: "
                f"{e164_phone or phone}"
            ),
            (
                "Country: "
                f"{country_name or country}"
            ),
            "",
            "Message:",
            message or "(none)",
    ...
    # CHANGE BY JYOTI - 11-Sep-2026:
    # Make verification one-use after successful submit.
    _consume_contact_email_verification(
        request
    )
